...
class WorkflowBuilder<
  Config extends WFConfig<unknown, ServiceRegistry, StepSchema>,
> {
  ...
  init<ID extends keyof Config["schema"]>(id: ID) {
    if (this.initIdx && this.initIdx > 0) {
      throw Error(`Only one "init" call per workflow is allowed`);
    }
    ...
    this.frontier = [this.idx];
    this.initIdx = this.idx;
    this.idToIdx[id as string] = this.idx;

    this.idx += 1;

    return this as WorkflowBuilder<Config>;
  }

  seq<ID extends keyof Config["schema"], R extends Config["schema"][ID]>(
    id: ID,

    resolve?: (ctx: ExprCtx<Config["services"], Config["schema"]>) => R,
  ...
  pipe<
    ID extends keyof Config["schema"],
    Mode extends PipeMode,
    Arr extends any[],
  >(
    id: ID,
    mode: Mode,
    input: (ctx: ExprCtx<Config["services"], Config["schema"]>) => Arr,

    builder: (
      b: WorkflowBuilder<
        WFConfig<Arr[number], Config["services"], Config["schema"]>
      >,
    ) => any,
    options?: StepOptions,
  ): WorkflowBuilder<Config> {
    ...
    const wfId = generateWorkflowId(id as string);
    ...
    const endSteps = built.steps.filter(
      (s: StepDef) => s.spec !== "__init__" && !hasDependents.has(s.idx),
    );
    ...
    this.steps.push({
      ...
      pipe: {
        ...
        // entryMap: Object.fromEntries(entrySteps.map((s) => [s.id, s.idx])),
        exitMap: endSteps.map((s: StepDef) => s.idx),
      },
      options,
    });
  ...
  par<Branches extends readonly WorkflowBuilder<Config>[]>(
    ...branches: {
      [K in keyof Branches]: (builder: WorkflowBuilder<Config>) => Branches[K];
    }
  ): WorkflowBuilder<Config> {
    const parentFrontier = [...this.frontier];
    let currentIdx = this.idx;
    ...
    branches.forEach((branch) => {
      const b = new WorkflowBuilder<Config>(this.name);
  ...
  join() {
    ...
    return this as any as WorkflowBuilder<Config>;
  }
  ...
  sub<Prefix extends string, SF extends WorkflowDef<any, any>>(
    ...
    sf: SF,
    resolve?: (
      ctx: ExprCtx<Config["services"], Config["schema"]>,
    ) => WorkflowInput<SF>,
  ): WorkflowBuilder<Config> {
    if (!sf) {
      throw new Error(`Subflow not found`);
  ...
  if<ID extends keyof Config["schema"], R extends Config["schema"][ID]>(
    id: ID,
    resolve: (ctx: ExprCtx<Config["services"], Config["schema"]>) => R,

    builder: (b: WorkflowBuilder<Config>) => any,
  ): WorkflowBuilder<Config> {
    this.seq(id, resolve);
  ...
  output<Out>(
    resolve: (ctx: ExprCtx<Config["services"], Config["schema"]>) => Out,
  ): WorkflowDef<Config["input"], Out> {
    const id = `${this.name}_out`;
    this.idToIdx[id] = this.idx;
    ...
    return this.build() as WorkflowDef<Config["input"], Out>;
  }
...
export function createWorkflow<Shared extends ServiceRegistry>() {
  return function workflow<Input = unknown, Schema extends StepSchema = {}>(
    name: string,
  ) {
    return new WorkflowBuilder<WFConfig<Input, Shared, Schema>>(name);
  };
}
