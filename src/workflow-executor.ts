...
export async function executeWorkflow<
  Reg extends ActionRegistry,
  WR extends Record<string, WorkflowDef<any, any, any, any, any>>,
  K extends keyof WR,
>(
  workflowId: K,
  workflowRegistry: WR,
  actionRegistry: Reg,
  input: WorkflowInput<WR[K]>,
  context: any,
  observers: WorkflowObserver<Reg>[] = [],
): Promise<{
  ...
  const workflow = workflowRegistry[workflowId] as WorkflowDef<
    any,
    any,
    any,
    any,
    ...
    await Promise.all(
      batch.map(async (stepId) => {
        ...
        const core = async () => {
          ...
          // -----------------------------
          if (step.__subflowId) {
            const subWorkflow = (step as any).__subflow as WorkflowDef<
              Reg,
              any,
              any
            >;
  ...
  return {
    ...
    output: output as WorkflowOutput<WR[K]>,
    extras,
  };
}
